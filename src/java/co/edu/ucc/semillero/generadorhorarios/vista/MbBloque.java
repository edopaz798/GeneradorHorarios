/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package co.edu.ucc.semillero.generadorhorarios.vista;

import co.edu.ucc.semillero.generadorhorarios.modelo.Bloque;
import co.edu.ucc.semillero.generadorhorarios.servicios.LogicaBloque;
import java.io.Serializable;
import java.util.List;
import javax.annotation.PostConstruct;
import javax.ejb.EJB;
import javax.faces.application.FacesMessage;
import javax.faces.bean.ManagedBean;
import javax.faces.bean.ViewScoped;
import javax.faces.context.FacesContext;

/**
 * Clase que repesenta el Managed Bean de la entidad Bloque
 *
 * @author Eduardo Paz
 * @version 1.0.0
 * @since 1.0.0
 */
@ManagedBean(name = "mbBloque")
@ViewScoped
public class MbBloque implements Serializable {

    @EJB
    private LogicaBloque logica;
    private Bloque bloque;
    private List<Bloque> lista;

    /**
     * Metodo que instancia la variable bloque e inicializa la variable lista
     * con todas las ocurrencias que tenga la tabla Bloque en la base de datos
     *
     * @since 1.0.0
     */
    @PostConstruct
    public void init() {
        bloque = new Bloque();
        lista = logica.consultarBloques();
    }

    /**
     * Metodo que ejecuta la accion de guardar el valor de la variable bloque en
     * la base de datos, genera un mensaje positivo o negativo dependiendo del
     * resultado de esta accion
     *
     * @return null para todos los casos
     * @since 1.0.0
     */
    public String accionGuardar() {
        FacesContext context = FacesContext.getCurrentInstance();
        if (logica.guardar(bloque)) {
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Correcto", "Se ha registrado correctamente"));
            init();
        } else {
            context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "Error al guardar"));
        }
        return null;
    }

    /**
     * Metodo No Implementado
     *
     * @param p
     * @return
     * @since 1.0.0
     */
    public String accionEditar(Bloque p) {
        this.bloque = p;
        return null;
    }

    /**
     * Metodo que elimina una incidencia en la base de datos
     *
     * @param p Bloque a eliminar
     * @return null para todos los casos
     * @since 1.0.0
     */
    public String accionEliminar(Bloque p) {
        logica.eliminar(p);
        init();
        return null;
    }

    /**
     * Metodo para obtener el valor actual de la variable bloque que es de tipo
     * Bloque
     *
     * @return retorna el valor de la variable bloque
     * @since 1.0.0
     */
    public Bloque getBloque() {
        return bloque;
    }

    /**
     * Metodo para guardar un valor de tipo Bloque en la variable bloque
     *
     * @param bloque Bloque a guardar
     * @since 1.0.0
     */
    public void setBloque(Bloque bloque) {
        this.bloque = bloque;
    }

    /**
     * Metodo para obtener el valor actual de la variable lista que es de tipo
     * List(Bloque)
     *
     * @return retorna el valor de la variable lista
     * @since 1.0.0
     */
    public List<Bloque> getLista() {
        return lista;
    }
}
